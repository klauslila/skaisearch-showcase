...
**21.8M+** price observations · **2.14M+** flights · **2,380k+** logged query attempts ·
**2,500+** collection runs · **44** active routes · **40** airports · **10.0 GB** on disk ·
...
The promoted model's typical error runs **33% below** a route×lead median baseline (RMSE **291** vs **434**), off by **13.6%** on average, fitted on
203,447 rows across 400 trees. Each figure is the median of 4 separate 14-day runs, so one lucky window can't carry it. On those same windows it scores R² 0.83 against a 0.58 baseline, quoted per-window because R² divides by each window's own variance and so isn't comparable between runs. Pushed to **49 days out** the reduction is **31%**, at **20.9%** error. Gain by feature:

| Feature | Gain | Feature | Gain |
|---|---|---|---|
| Trip type | ██████████ 37.7% | Climate season | ▍ 1.8% |
| Distance / haul | ███████▏ 27.3% | Source | ▍ 1.5% |
| Destination | ███▊ 14.2% | Lead time (days) | ▎ 1.1% |
| Stops | ██▍ 9.2% | Day of week | ▏ 0.8% |
| Departure month | ▉ 3.5% | Carriers | ▏ 0.8% |
| Origin | ▌ 2.2% |  |  |
<!-- stats:end:model -->
